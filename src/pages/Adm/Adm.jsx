...
import { fetchVenues, upsertVenue, upsertShow } from "../../data/api";
...
const Adm = () => {
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    try {
      const [fetchedVenues] = await Promise.all([fetchVenues()]);
    ...
    <main className={styles.container}>
      ...
      <section className={styles.formSection}>
        ...
        <form onSubmit={handleVenueSave} className={styles.form}>
          ...
          />
          <input
            placeholder="googleLink"
            value={venueForm.googleLink}
            onChange={(e) =>
              setVenueForm({ ...venueForm, googleLink: e.target.value })
            }
          />
          <input
            placeholder="appleLink"
            value={venueForm.appleLink}
            onChange={(e) =>
              setVenueForm({ ...venueForm, appleLink: e.target.value })
            }
          />
          <div>
            <button className={`${components.btn}`} type="submit">
              Salvar
            </button>
            <button
              type="button"
              onClick={() => setVenueForm(emptyVenue)}
      ...
      <section className={styles.formSection}>
        ...
        <form onSubmit={handleShowSave} className={styles.form}>
          <input
            ...
            value={showForm.dateTime}
            onChange={(e) =>
              setShowForm({ ...showForm, dateTime: e.target.value })
            }
            required
          />
          <input
            placeholder="Dia da Semana (Seg, Ter...)"
            value={showForm.weekDay}
            onChange={(e) =>
              setShowForm({ ...showForm, weekDay: e.target.value })
            }
            required
          />
          <input
          ...
          />
          <input
            placeholder="dia (1, 2, 20...)"
            value={showForm.day}
            onChange={(e) => setShowForm({ ...showForm, day: e.target.value })}
          />
          <input
            placeholder="mês (Jan, Fev...)"
            value={showForm.month}
            onChange={(e) =>
              setShowForm({ ...showForm, month: e.target.value })
            }
          />
          <input
            placeholder="ano (2025)"
            value={showForm.year}
            onChange={(e) => setShowForm({ ...showForm, year: e.target.value })}
          />
          <select
